...
function Nova() {
  ...
  const handleCommand = (cmd: string) => {
    if (cmd.includes("check email") || cmd.includes("read email")) {
      ...
      speakWithAvatar("You have 2 meetings today. 10am with Client, and 3pm team sync.");
    } else if (cmd.includes("set meeting") || cmd.includes("schedule meeting")) {
      setActive("meeting");
      setStatus("Let's set a meeting");
      speakWithAvatar("Who is the meeting with and what time?");
    } else if (cmd.includes("summarize")) {
      setActive("meeting");
      speakWithAvatar("Please paste your meeting notes and I will summarize them.");
    } else {
      speakWithAvatar("Try saying: check email, check schedule, or set meeting");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-[#F5F5F5] font-sans overflow-hidden">
      ...
      {/* 3D HOLOGRAM AI FACE — NO SQUARE, PURE HOLOGRAM */}
      <div className="fixed bottom-6 left-6 z-50">
        <div className="relative flex flex-col items-center">
          <div className="absolute inset-0 rounded-full bg-[#4F46E5] blur-3xl opacity-60"></div>
          <div className="absolute inset-0 rounded-full bg-[#FBBF24] blur-2xl opacity-40"></div>
          <video
            ref={videoRef}
            src={novaVideo.url}
            loop
            muted
            playsInline
            className={`relative w-48 h-48 object-cover mix-blend-screen transition-all duration-300 ${
              isTalking
                ? "animate-bounce scale-105 drop-shadow-[0_0_40px_#FBBF24]"
                : "drop-shadow-[0_0_25px_#4F46E5]"
            }`}
          />
          <div className="mt-4 flex flex-col items-center gap-2">
            <div className="flex items-center justify-center gap-2">
              <Crown size={14} className="text-[#FBBF24]" />
              <p className="text-[#F5F5F5] font-bold text-sm tracking-wider">NOVA</p>
            </div>
            <p className="text-xs text-[#10B981] text-center">{status}</p>
            <button
              onClick={startListening}
              className={`px-5 py-2 rounded-full font-bold flex items-center justify-center gap-2 transition shadow-lg backdrop-blur-xl border ${
                isListening
                  ? "bg-[#E11D48] text-white border-[#E11D48] shadow-[0_0_20px_#E11D48] animate-pulse"
                  : "bg-[#FBBF24]/20 text-[#FBBF24] border-[#FBBF24]/50 hover:bg-[#FBBF24]/30"
              }`}
            >
        ...
        {active === "dashboard" && (
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              <GlowCard title="Executive Voice" desc="3D Avatar + Premium Female Voice + Voice Commands" color={COLORS.royal} />
              <GlowCard title="Smart Email" desc="AI drafts + reads emails aloud" color={COLORS.gold} />
              <GlowCard title="Auto Scheduler" desc="Plan and set meetings with voice" color={COLORS.emerald} />
            </div>
            <button
              onClick={() => speakWithAvatar("Good day. I am Nova, your executive assistant.")}
              className="bg-gradient-to-r from-[#FBBF24] to-[#4F46E5] text-black px-6 py-3 rounded-xl font-bold flex items-center gap-2"
            >
        ...
        {active === "email" && (
          <div>
            ...
            <div className="backdrop-blur-2xl bg-white/5 border border-white/10 p-6 rounded-2xl">
              ...
              />
              <button
                onClick={() => speakWithAvatar(emailText)}
